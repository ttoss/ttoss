...
**Acceptance:** one fixture and test per new `IntentIssueCode` except `ambiguous-dataset`/`no-matching-dataset` (Phase 3) — including `unknown-category`'s two triggering paths (`categoryId` absent, `categoryId` present but unlisted) and a fixture confirming a non-nominal metric with an extra `categoryId` still validates (D6's "inert, not an error" case); `datasetId`-supplied and `datasetId`-omitted-but-unambiguous paths both tested; a schema-version-mismatch fixture confirms the `set-value` repair suggests `INTENT_SCHEMA_VERSION`.

...
describe('createThemeRuntime', () => {
  ...
  describe('short-circuit', () => {
    test('setTheme with same value does not notify', () => {
      runtime = createThemeRuntime({
        defaultTheme: 'bruttal',
        defaultMode: 'light',
      });
  ...
  describe('input sanitization', () => {
    test('setTheme ignores empty string', () => {
      runtime = createThemeRuntime({
        defaultTheme: 'bruttal',
        defaultMode: 'light',
      });
      ...
      runtime.setTheme('');
      expect(states).toHaveLength(0);
      expect(runtime.getState().themeId).toBe('bruttal');
    });

    test('setTheme ignores non-string value', () => {
      runtime = createThemeRuntime({
        defaultTheme: 'bruttal',
        defaultMode: 'light',
      });
      runtime.setTheme(123 as unknown as string);
      expect(runtime.getState().themeId).toBe('bruttal');
    });

    test('setTheme ignores themeId with special characters', () => {
      runtime = createThemeRuntime({
        defaultTheme: 'bruttal',
        defaultMode: 'light',
      });
      ...
      runtime.setTheme('foo"bar');
      expect(states).toHaveLength(0);
      expect(runtime.getState().themeId).toBe('bruttal');
    });

    test('setTheme ignores themeId with spaces', () => {
      runtime = createThemeRuntime({
        defaultTheme: 'bruttal',
        defaultMode: 'light',
      });

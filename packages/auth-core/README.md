...
Three helpers own the RFC 9728 document — its shape, where it is served, and the `WWW-Authenticate` value that points clients at it. They live here so every consumer derives the same answers; `oauthServer`, `createProtectedResourceMetadataMiddleware` and `@ttoss/http-server-mcp`'s router all build on them.

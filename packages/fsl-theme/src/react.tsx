...
// ---------------------------------------------------------------------------
// Density projection (ADR-019)
// ---------------------------------------------------------------------------

/** The theme density axis — the geometry projection set via `data-tt-density`. */
export type Density = 'compact' | 'comfortable' | 'spacious';

const DensityCtx = React.createContext<Density>('comfortable');

/** Props for {@link DensityProvider}. */
export interface DensityProviderProps {
  /** The density to apply to this subtree. */
  density: Density;
  /**
   * Element that carries the `data-tt-density` scope. It uses
   * `display: contents` so it adds no box — CSS custom properties still
   * inherit through it — keeping layout untouched.
   * @default 'div'
   */
  as?: 'div' | 'section' | 'main';
  /** The subtree that inherits this density. */
  children: React.ReactNode;
}

/**
 * Projects a **density** onto its subtree — the geometry analogue of colour
 * mode (ADR-019). Set it once near the root, or nest it to make a region
 * (a data table, a token tree) denser than the app default. Components do not
 * change: they read the semantic geometry tokens, which the `[data-tt-density]`
 * CSS remaps. Density tunes control inset only; the touch-pointer hit floor
 * (ADR-020) always wins, so accessibility is never affected.
 *
 * @example
 * ```tsx
...
export const DensityProvider = ({
  ...
  return (
    <DensityCtx.Provider value={density}>
      <Tag data-tt-density={density} style={{ display: 'contents' }}>
        {children}
      </Tag>
    </DensityCtx.Provider>
  );
...
/**
 * Read the current {@link Density} from the nearest {@link DensityProvider}.
 * Returns `'comfortable'` (the default) when no provider is present.
 */
export const useDensity = (): Density => {
  return React.useContext(DensityCtx);
};

...
describe('computeFilterDomain', () => {
  ...
  test('the computed domain is a value the application holds alongside the filter, not the catalog domain itself', () => {
    const domain = computeFilterDomain({
      filter: numeric,
      rows: [{ populacao: 1 }, { populacao: 2 }],
    });

    expect(domain).toEqual({ mode: 'range', min: 1, max: 2 });
    expect(numeric.domain).toEqual({ mode: 'runtime' });
  });
});

...
describe('validateCatalog', () => {
  ...
  describe('options.capabilities (mapType/adapter geometry intersection)', () => {
    test('omitting capabilities skips the check entirely', () => {
      const result = validateCatalog(sampleCatalog);
      expect(result.status).toBe('valid');
    });

    test('a capability set covering every declared geometry stays valid', () => {
      const result = validateCatalog(sampleCatalog, {
        capabilities: {
          sourceTypes: ['geojson'],
          layerGeometries: ['point', 'line', 'polygon'],
    ...
    test('unsupported-map-type-geometry: a map type needing a geometry the adapter cannot render is a mismatch', () => {
      const result = validateCatalog(sampleCatalog, {
        capabilities: {
          sourceTypes: ['geojson'],
          layerGeometries: ['line'],
          dataFeatures: { featureState: ['geojson'], filter: ['geojson'] },
          viewFeatures: { pitch: false, bearing: false },
        },
      });
      ...
      if (result.status !== 'valid') {
        const issue = result.issues.find((candidate) => {
          return candidate.code === 'unsupported-map-type-geometry';
        });
        expect(issue).toBeDefined();

...
| `data-orientation` | `ButtonGroup` only                          | `"horizontal" \| "vertical"`                                        | Always. Reflects the axis **actually rendered**, so it reads `vertical` when a horizontal row had to collapse — measure the rendered state, not the authored prop. |

...
Tags: feedback, inline-alert, status.passive, a11y, live-region, valence, consumes:fsl-theme-ADR-029, closes:F-062, opens:F-063
...
**The posture, extracted rather than inlined.** `src/tokens/passiveStatus.ts` states it once: root fill `feedback.muted.background.default`, root edge `feedback.muted.border.default`, prose `feedback.muted.text.default`, valence confined to the mark. `StatusLight` is its **second consumer and it already shipped** — a dot inked from the valence beside a label inked from the page, no fill (ADR-038). That satisfies the two-consumer gate F-053 and F-058 both apply before admitting a shared source, retroactively rather than aspirationally, and it turns `StatusLight` from a one-off into the chip-scale member of a posture. `ProgressBar`/`Meter` are the mirror rule already in the codebase (quiet track, valence fill).

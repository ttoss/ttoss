...
| **3. Fixed ramp or `cqi`?** (F-021, F-035)                                    | **Read against our own docs 2026-07-29 (owner's framing: S2 is reference only; what do _our_ diretrizes rule?), and they already rule most of it.** `sizing.md`: ergonomics by pointer profile, coarse never fluid — `hit` obeys. `spacing.md`: fluid-from-one-engine is the family's **identity**, and bounded `clamp` is the sanctioned, validation-enforced contract where the resolved outcome is a guarantee (`gutter.*`, `separation.interactive.min`; `inset.action.block` bounded 8–9px "so a CTA resolves ~40px"). `typography.md`: control type fluid by design, no pointer hook — F-021's step-up half is genuinely undecided but is _our_ identity question, not an S2 import. **What the reading added:** the theme conforms to ADR-020 (`inset.control` = `core.spacing.{1                                                                                                                                                                                                                                                                                                                                                                                                                                       | 2   | 4}`); `spacing.md`'s table claimed `{3 | 4   | 6}`— stale, fixed same day; and ADR-020's premise ("the ±2px residual **never binds**,`hit`drives height") is false above ~900px — exactly F-035's 32→34. **The remaining decision is a classification, not doctrine:** (a) control inset is outcome-bearing → bound`inset.control.\*` (data-only + one contract line), making ADR-019/020 true by mechanism — recommended; or (b) narrow the ruling in writing to "the floor is not fluid". F-021's coarse-type half: keep our fluid type (identity), readmit on evidence of illegibility or a coarse-pointer consumer. |

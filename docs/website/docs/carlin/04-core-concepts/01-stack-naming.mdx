...
1. **Unicode normalization**: Accented characters are decomposed and their diacritical marks are stripped, mapping them to ASCII equivalents (e.g. `configuração` → `configuracao`, `não` → `nao`).

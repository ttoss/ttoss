...
Branch names are automatically converted to param-case (lowercase with hyphens). Non-ASCII characters (such as accented letters) are normalized to their ASCII equivalents before the stack name is generated.
...
1. Unicode characters are decomposed using NFKD normalization (e.g. `ç` → `c` + combining cedilla).

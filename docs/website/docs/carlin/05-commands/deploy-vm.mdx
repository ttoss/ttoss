...
- Pipes deployment script content directly through SSH to bash
- Optionally fixes SSH key file permissions before connecting
- Executes scripts remotely and streams output to console
...
flowchart TD
    ...
    C --> F{Fix SSH Key Permissions?}
    F -->|Yes| G[Adjust SSH Key Permissions (chmod 400)]
    F -->|No| E[Establish SSH Connection]
    G --> E
    D --> E[Establish SSH Connection]
    E --> H[Pipe Script Content to SSH stdin]
    H --> I[Execute Script via bash -s]
...
| `Permission denied` on execute  | Script not executable on remote VM | Ensure script has execute permissions on the remote VM or modify script to be executable                                                  |

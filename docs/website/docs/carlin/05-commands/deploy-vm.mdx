...
:::info Configuration File Support

Configuration via `carlin.yml` (e.g., `vmUserName`, `vmHost`, `vmKeyPath`, etc.) is not yet implemented. Currently, all options must be provided as CLI arguments. Configuration file support is planned for a future release.
...
:::warning Password Authentication Limitations
...
flowchart TD
    ...
    E --> H[Stream Script via SSH stdin]
    H --> I[Execute Script with bash -s]

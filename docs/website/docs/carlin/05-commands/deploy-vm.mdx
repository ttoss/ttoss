...
| `--fix-permissions` | `boolean` | ❌       | `false` | Automatically fix script execution permissions    |
...
| `Host key verification failed`  | SSH host key not trusted           | Add host to `~/.ssh/known_hosts` or use `StrictHostKeyChecking=no` (not recommended for production) |

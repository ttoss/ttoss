...
Once <Carlin /> upload the code to S3, it sets the `LambdaS3Bucket`, `LambdaS3Key` and `LambdaS3ObjectVersion` [parameters](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/parameters-section-structure.html) to the CloudFormation template:
...
<TabItem value="yaml" label="Yaml">
  ```yml Parameters: LambdaS3Bucket: Type: String Description: The S3 bucket
  where the Lambda code is stored. LambdaS3Key: Type: String Description: The S3
  key where the Lambda code is stored. LambdaS3ObjectVersion: Type: String
  Description: The S3 object version of the Lambda code. ```
</TabItem>

</Tabs>

With those paramenters defined, you can set the [`Code`](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-lambda-function-code.html)
...
of the [AWS::Serverless::Function resource](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-resource-function.html), referencing the `LambdaS3Bucket`, `LambdaS3Key` and `LambdaS3ObjectVersion` parameters in your template. If `Code` or `CodeUri` is not set, <Carlin /> will automatically set them to the values of the parameters, as shown in the following example (if `Code` or `CodeUri` is defined in the template, <Carlin /> won't modify them):
...
{' '}
<TabItem value="yaml" label="Yaml">
  ```yml Resources: LambdaFunction: Type: AWS::Lambda::Function Properties:
  Code: S3Bucket: !Ref LambdaS3Bucket S3Key: !Ref LambdaS3Key S3ObjectVersion:
  !Ref LambdaS3ObjectVersion ServerlessFunction: Type: AWS::Serverless::Function
  Properties: CodeUri: Bucket: !Ref LambdaS3Bucket Key: !Ref LambdaS3Key
...
To overcome this problem, destroy will only delete the resources if termination protetion isn't enabled and if <OptionAliasesInline options={cliOptions} option="environment" to="/docs/carlin/commands/deploy#global-options" /> isn't defined.
...
<OptionsTable options={cliOptions} />

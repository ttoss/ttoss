...
Data Visualization validation protects the **category-level semantic contract**.

It validates only what spans colors, encodings, and geospatial semantics.
...
- `dataviz.geo.*` introduces a parallel color or encoding language instead of reusing:
  - `dataviz.color.*`
  - `dataviz.encoding.*`
...
- forecast or uncertainty is introduced as a standalone semantic token instead of composition across dataviz families
...
- the category exposes color semantics without exposing non-color encoding primitives, increasing the risk that meaning depends only on color

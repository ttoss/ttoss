...
They model different dimensions and may lawfully co-exist. `status.interruptive` says _how_ the entity operates; `interruptive` says _what impact_ it has on the user. An expression can carry both simultaneously without redundancy.

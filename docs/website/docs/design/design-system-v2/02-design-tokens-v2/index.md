...
To begin using tokens, read [Token Model](./model.md) to understand the contract between core and semantic tokens, then review [Governance](./governance.md) for change management. Each token family has its own page starting with [Colors](./categories/colors.md).

...
| Input      | `TextField`, `TextArea`, `SearchField` (each with `*Label` / `*Control` / …), `NumberField`, `Slider`, `ComboBox` / `ComboBoxItem` (typeahead-filtered list — a freeform channel makes a picker Input, ADR-012)                                                                                |
...
Waves 1 and 2 of the full React Aria atomic catalog are complete, and Wave 3 has landed `Table` and `ComboBox`; `Tree` and the date/time suite follow under the same pattern. Every component lands with contract tests, keyboard tests, and an axe accessibility suite.

...
Analytical color comes from `core.colors.*` — the same palette foundation used by the UI.

This keeps color values in a single location and makes dataviz palettes automatically consistent with the brand.
...
Analytical colors come from `core.colors.*` — the same foundation palette used by UI semantics.

The hue families used by dataviz (brand, red, orange, yellow, green, teal, purple, pink, neutral) must be provided in `core.colors` when building the theme.

Core color tokens must remain context-free and value-only.

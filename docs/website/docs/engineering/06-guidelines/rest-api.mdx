...
- Use the `src/api` directory to define your API resources.
- Create folders brackets `{}` to define dynamic routes. For example, `src/api/user/{id}/route.ts` will be the route for `GET /user/{id}`.
- Use the `route.ts` file to define the methods for the resource.
- Name your methods with the HTTP method in uppercase. For example, `GET`, `POST`, `PUT`, `DELETE`.
- Use the `APIGatewayProxyHandler` type from `aws-lambda` to define the method signature.
  - Don't forget to install [`@types/aws-lambda`](https://www.npmjs.com/package/@types/aws-lambda).
- Name your CloudFormation function resources with the following pattern: `{ResourceName}{HTTPMethod}Function`. Examples:
  - `UsersGETFunction`: `GET /users`
  - `UserIdGETFunction`: `GET /user/{id}`
- Use the `Globals` property to define common properties for all functions.

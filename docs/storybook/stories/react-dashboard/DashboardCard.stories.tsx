...
export const LineChartWithSparkline: StoryObj = {
  render: () => {
    return (
      <Box sx={{ maxWidth: '400px', padding: '4' }}>
        <DashboardCard
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Card with `type: "lineChart"` renders a `BigNumberSparkline` — a large metric number with an inline SVG sparkline below it. Uses `data.meta.daily` (or `data.api.daily`) as the time-series data.',
      },
    },
  },
};

export const LineChartWithPreviousPeriod: StoryObj = {
  render: () => {
    return (
      <Stack
        sx={{
        ...
        <Box sx={{ width: '320px' }}>
          <DashboardCard
            ...
            data={{
              meta: {
                total: 0.72,
                daily: [0.85, 0.80, 0.78, 0.75, 0.73, 0.70, 0.72],
                dailyPrevious: [0.90, 0.88, 0.86, 0.84, 0.82, 0.80, 0.79],
              },
            }}
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Sparkline with `dailyPrevious` data renders a dashed overlay line for the previous period comparison. The current period line uses green for positive trends and red for negative.',
      },
    },
  },
};
